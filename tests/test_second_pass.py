...
class OnlyRoomRentDrivesTheDeductionTest(unittest.TestCase):
    """The breach has to be a room breach. This was a real bug, twice."""

    def icu_over_limit(self):
        return (
            BillLine(item="icu charges 12,000 x 2 days", amount=24000, qty=2),
            LineVerdict(
                item="icu charges 12,000 x 2 days",
                charged=24000,
                allowed=10000,
                clause_id="II.1",
                reason="capped",
    ...
    def test_an_icu_breach_does_not_set_the_ratio(self):
        ratio, source = breach_ratio([self.icu_over_limit()[0]], [self.icu_over_limit()[1]])
        self.assertEqual(ratio, 1.0)
        self.assertIsNone(source)

    def test_a_surgeon_fee_breach_does_not_set_the_ratio(self):
        line = BillLine(item="surgeon fee", amount=95000, qty=1)
        verdict = LineVerdict(
            item="surgeon fee",
            charged=95000,
            allowed=5000,
    ...
    def test_the_room_line_still_sets_it_when_both_are_present(self):
        icu_line, icu_verdict = self.icu_over_limit()
        room_line, room_verdict = room()
        ratio, source = breach_ratio([icu_line, room_line], [icu_verdict, room_verdict])
        self.assertAlmostEqual(ratio, 0.625)
        self.assertEqual(source, "room rent")

    def test_nothing_is_rescaled_when_only_a_non_room_line_breached(self):
        pairs = [self.icu_over_limit(), other("surgeon fee", 100000.0)]
