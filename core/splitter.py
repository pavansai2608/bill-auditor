...
def _without_the_address_block(clause: Clause) -> Clause | None:
    ...
    **DO NOT SPLIT WHAT IS LEFT INTO TABLE AND PROSE.** It is tempting: the
    surviving body is 12,414 characters, and breaking it where the rendered rows
    stop yields a tidy 9,748-character grid and a 2,665-character legend. That
    was tried, in BA-240, and reverted in BA-242. The legend is the definition
    of the grid - "'Not Covered' means that particular benefit is NOT available
    either as an inbuilt feature or as an optional feature in that particular
    plan" - and separated from it, retrieval brings back one without the other:

    - Measured over nine queries, the two arrived together twice. The agent's
      own `QUERY_ANGLES["other"]` angle 1, "<item> limit coverage", returned the
      grid at rank 2 with **the legend nowhere in the top 25** - so this is not
      a `rerank_top_n` budget that widening would fix.
    - `refs` is empty on both, so `with_references` has no citation to follow.
    - The legend names its target positionally, as "Key to read **above
      table**". Once they are separate records nothing in the index can resolve
      "above".

    A judge handed the grid alone reads "Not Covered" cells with no key. That is
    exactly how B21 and B28 produced a confident Rs 0 on lines the answer key
    pays in full, and `KNOWN_LIMITATIONS.md` section 10 is about the same hole.
    Splitting these two reintroduces that failure in a form that is harder to
    see, because the semantics are then missing from a different clause rather
    than from this one.

    The size is legal on its own terms: `tests/test_ingest.py` caps **prose**,
    not total characters, because under-split prose is what swamps `num_ctx`.
    This clause is 2,666 characters of prose against a 12,000 ceiling and 12,414
    total against a 16,000 ceiling. Both with room. Nothing about its length
    requires it to be broken up.
    """
