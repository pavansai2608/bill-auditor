...
def breach_ratio(lines: list[BillLine], verdicts: list[LineVerdict]) -> tuple[float, str | None]:
    ...
    `over_limit` is set only by a breached per-day cap, which is exactly the
    breach that triggers this. Where more than one room line breached, the
    lowest ratio wins - the insured cannot be better off for having been billed
    the same room twice.
    """
