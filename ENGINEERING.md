...
How this system is built and why, written as the reasons were found rather than
as a specification. Most of what follows is here because something was wrong
first: the splitter reads two-column PDFs the way it does because reading
straight across produced nonsense, the LLM never does arithmetic because an 8B
model got it wrong invisibly, and the E2E stage owns its own script because the
version before it went green against a build nobody had made.

Operating rules for working in this repository are in `CLAUDE.md`. The phase
plan is `PHASES.md`. Accuracy numbers are authoritative only in
`eval/results.md`, and `KNOWN_LIMITATIONS.md` says what they do and do not mean.
...
54.9% in the ladder table of `PHASES.md` (lines 159-160) are not reproducible
...
## The pipeline, and the three times it reported work it had not done
...
### Pruning, and the keep rule that is not arithmetic

`ci/prune_images.py` plus a `Prune` stage after Deploy, main only, gated on the
whole ledger including `Deploy`. Every build makes five images; without this the
disk grows one set per build for ever.

A tag is kept if it is N, N-1, `latest`, **or referenced by a live Kubernetes
deployment - whatever its number**. The fourth rule outranks the other three.
The cluster does not necessarily run what the build just pushed: before
`k8s/deploy.sh`, `kubectl apply` started no rollout at all, and pruning on the
numbers alone would have deleted the image the cluster was actually serving.
Live references are read from the cluster at prune time.

Only repositories under `bill-auditor/` are ever candidates. Not ollama, not the
base images, not the other projects on this machine, and **not the compose
images** - `docker compose` names them `bill-auditor-gateway` with a hyphen,
a different repository from `bill-auditor/gateway`, and that is what the local
stack runs.

Both daemons are pruned, because Docker Desktop's and minikube's image stores
are separate - the same separation that made Deploy silently no-op. The minikube
profile is resolved from `minikube profile list -o json`, never hardcoded, and
if minikube is not running that half is skipped rather than failing the build.

A blocked or failed Prune does not fail the build. The images are still correct
